...
public class Day6Part2Optimized {
    ...
    private static Long numberOfWays(Long time, Long dist) {
        Long numberOfWaysNotPossible =0L;
        Long totalNumberOfWays =time+1;
        long t;
        for(t =0;t<=time;t++){
            if(t*(time-t)<=dist) numberOfWaysNotPossible++;
            else break;
        }
        for(long t_rev =time;t_rev>t;t_rev--){
            if(t_rev*(time-t_rev)<=dist) numberOfWaysNotPossible++;
            else break;
        }
        long numberOfWays =(totalNumberOfWays-numberOfWaysNotPossible);
...
multiply of numbers : 35865985, time taken: 152
* */