        ...
        for(int i =0; i< row; i++){
            for (int j=0;j<col;j++){
                if (isSymbol(engineSchematic.get(i).charAt(j))){
                    ans += getGearRatio(engineSchematic,i,j);
                }
            }
        }
...
//            sumOfNumbers += getNumber(engineSchematic,i-1,j);
            numbers.add(getNumber(engineSchematic,i-1,j));
        }
...
//            sumOfNumbers += getNumber(engineSchematic,i,j-1);
            numbers.add(getNumber(engineSchematic,i,j-1));
        }
