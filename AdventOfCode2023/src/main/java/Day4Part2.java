...
public class Day4Part2 {
    ...
    public static void main(String[] args) {
        ...
        int[] cardCount = new int[scratchcards.size()];
        Arrays.fill(cardCount,1);

        for(int i=0;i<scratchcards.size();i++){

            int matches = getMatches(scratchcards.get(i));
            for(int j=i+1;j<scratchcards.size() && matches>0;j++,matches--){
                cardCount[j]+=cardCount[i];
            }
        }
        System.out.println("total cards : " + Arrays.toString(cardCount));
        System.out.println("total card sum : " + Arrays.stream(cardCount).sum());

    }

//    private static int process(String scratchcard) {
//        int ans =0;
//
